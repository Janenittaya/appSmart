...
@IonicPage() 
@Component({
  selector: 'page-activity-rice-compost',
  templateUrl: 'activity-rice-compost.html',
})
export class ActivityRiceCompostPage {
 ...
 loading.present();
 let ob = this.http.get('https://traceability.digitalsmartfarm.com/api/activity_compost.php?cropID='+this.corpInfo.id)
 ob.subscribe(
   (data: any) => {
       loading.dismiss();
       this.news = data;
...
actionConfirm() {
  this.http.get('https://traceability.digitalsmartfarm.com/api/saveaddcompostrice.php').subscribe((resp) => {
    if (resp) {
