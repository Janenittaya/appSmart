...
@IonicPage()
@Component({
  selector: 'page-activityharves',
  templateUrl: 'activityharves.html',
})
export class ActivityharvesPage {
  public event = {
    month: '2018-01-01',
  }
  ...
  ionViewDidEnter() {
    ...
    loading.present();
    let ob = this.http.get('https://traceability.digitalsmartfarm.com/api/activity_harves.php?cropID='+this.corpInfo.id)
    ob.subscribe(
      (data: any) => {
        loading.dismiss();
        this.news = data;
